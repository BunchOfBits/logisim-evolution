 ...
 * Model based on <a href="https://www.ti.com/lit/ds/symlink/sn74ls194a.pdf">74194 datasheet</a>.
 */
public class Ttl74194 extends AbstractTtlGate {
  ...
  // IC pin indices as specified in the datasheet for the DIP package
  ...
  private static final byte[] INPUTS = new byte[] { A, B, C, D };
  private static final byte[] OUTPUTS = new byte[] { QA, QB, QC, QD };
  ...
  private ShiftRegisterData getData() {
    ...
    if (data == null) {
      data = new ShiftRegisterData(BitWidth.ONE, INPUTS.length);
      _state.setData(data);
    }
      ...
      switch (getMode()) {
        case LOAD:
          for (var i = 0; i < INPUTS.length; i++) {
            getData().set(i, getPort(INPUTS[i]));
          }
          break;
        case SHIFT_LEFT:
  ...
  private void propagateOutputs() {
    for (var i = 0; i < OUTPUTS.length; i++) {
      setPort(OUTPUTS[i], getData().get(i));
    }
  }
